...
self.pr_now()))
    ...
    def receive_message(self, peer):
        ...
        try:
            ...
            print('{}: received {} [{}]'.format(self.pr_sock(peer), message_name, 
self.pr_now()))
        except ConnectionError as err:
            # a ConnectionError means it will never succeed
            print('closing: {}'.format(err))
    ...
    def get_connection(self, member):
        ...
        Note that the connection will be done in non-blocking mode, so may not be 
        available immediately--use select to pick it up when it becomes writable.
    ...
    def is_expired(self, peer=None, threshold=ASSUME_FAILURE_TIMEOUT):
        ...
        :return: True if elapsed time is past threshold, False otherwise (or if 
        state is quiescent)
        """
    ...
    def get_state(self, peer=None, detail=False):
        ...
        :return: either the state or (state, timestamp) depending on detail (not 
...
None)
    ...
    def set_state(self, state, peer=None, switch_mode=False):
        ...
        :param peer: socket connected to peer process (None means self)
        :param switch_mode: True if we want to move from read to write or vice 
        versa FIXME: autodetect this?
        """
    ...
    def send(cls, peer, message_name, message_data=None, wait_for_reply=False, buffer_size=BUF_SZ):
        """
        Pickles and sends the given message to the given socket and unpickles the 
        returned value and returns it.
        ...
        """
        message = message_name if message_data is None else (message_name, 
message_data)
    ...
    def join_group(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as gcd:
            ...
            self.members = self.send(gcd, 'JOIN', message_data, 
wait_for_reply=True)
